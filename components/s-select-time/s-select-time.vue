...
export default {
    ...
    methods: {
		moment(e){
			console.log(e)
			return Number(Date.parse(e.replace('-','/')))
		},
        timeAdd (time, minute) {
        ...
        hadleData () { // 处理数据
            ...
            while (this.moment(`${this.currentDate} ${this.timeAdd(start, this.interval)}`) <this.moment(`${this.currentDate} ${this.timeAdd(end, 0)}`)) {
                const item = {
                    time: this.timeAdd(start, this.interval),
                    selected: false,
                    disabled: this.isDisabled && this.moment(`${this.currentDate} ${this.timeAdd(start, this.interval)}`) < this.moment(this.currentTime),
                    range: false,
                    rangeHelp: false,
                    start: false,
                    end: false
            ...
            const starteItem = {
                ...
                selected: false,
                disabled: this.isDisabled && this.moment(`${this.currentDate} ${this.timeAdd(this.startTime, 0)}`) < this.moment(this.currentTime),
                range: false,
                rangeHelp: false,
                start: false,
                end: false
            ...
            const endItem = {
                ...
                selected: false,
                disabled: this.isDisabled && this.moment(`${this.currentDate} ${this.timeAdd(this.endTime, 0)}`) < this.moment(this.currentTime),
                range: false,
                rangeHelp: false,
                start: false,
                end: false
        ...
        selectTime (item) { // 选择时间
            ...
            if (this.selectedTime.length < 2) {
                ...
                const arr = []
                if (this.moment(`${this.currentDate} ${timeStrat}`) > this.moment(`${this.currentDate} ${timeEnd}`)) {
                    arr[0] = timeEnd
                    arr[1] = timeStrat
                } else {
                    arr[0] = timeStrat
                ...
                for (const itemTime of this.timeList) {
                    ...
                    if (this.moment(`${this.currentDate} ${itemTime.time}`) > this.moment(`${this.currentDate} ${arr[0]}`) && this.moment(`${this.currentDate} ${itemTime.time}`) < this.moment(`${this.currentDate} ${arr[1]}`)) {
                        itemTime.range = true
    ...
    created () {
        ...
        if (this.disabledTime.length > 0) {
            for (const item of this.disabledTime) {
                const startT = this.moment(`${this.currentDate} ${this.timeList[0].time}`)
                const endT = this.moment(`${this.currentDate} ${this.timeList[this.timeList.length - 1].time}`)
                const start = this.moment(`${this.currentDate} ${this.timeAdd(item.start, 0)}`)
                const end = this.moment(`${this.currentDate} ${this.timeAdd(item.end, 0)}`)
                if (start > startT && start < endT) {
                    for (const timeItem of this.timeList) {
                        const itemTime = this.moment(`${this.currentDate} ${timeItem.time}`)
